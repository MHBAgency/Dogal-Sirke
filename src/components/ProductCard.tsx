...
export function ProductCard({ title, description, price, imageSrc, type }: ProductCardProps) {
    const { addToCart } = useCart();
    const [quantity, setQuantity] = React.useState(1); // Added quantity state

    const handleAddToCart = () => {
        addToCart({
            id: type, // using type as ID since we have 3 distinct products
            title,
            price,
            imageSrc,
            quantity, // Added quantity to cart item
        });
